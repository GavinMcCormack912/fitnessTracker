...
print(df.columns[:3].tolist() + ['label'])
...
# ----------------------------------------------------------------
# Choose method, deal with outliers
# ----------------------------------------------------------------
col = "gyr_z"
dataset = mark_outliers_chauvenet(df, col)
dataset[dataset["gyr_z_outlier"]]

#this will remove the gyr_z values of columns with outliers for the column!
dataset.loc[dataset["gyr_z_outlier"], "gyr_z"] = np.nan


# Make a loop. apply np.nan to all outlier values!
...
for col in outlier_columns:
    for label in df["label"].unique():
        ...
        print(f"Removed {n_outliers} frmo column {col} for exercise {label}")
